...
var beginTest = function () {
  ...
  lastIndex = 0;
  $("#test-results").html(
    '<h6>Test Results <small class="text-muted">' +
...
var showTestResult = function (test) {
  ...
  if (test.responseCode !== 0) {
    resp += test.responseCode + "\n\n";
  }

  var responseHeadersStr = JSON.stringify(test.responseHeaders, null, "    ");

  if (responseHeadersStr !== "{}") {
    resp += "Headers:\n" + JSON.stringify(test.responseHeaders, null, "    ");
  }

  if (test.report !== undefined) {
    resp += "\n" + "Errors:\n" + JSON.stringify(test.report, null, "    ");
  }
  ...
  if (test.responseBody !== undefined) {
    resp += "\n\n" + "Response:\n" + JSON.stringify(test.responseBody, null, "    ");
  }
...
showTestResults = function () {
  $.get(
    "/test/status?runId=" + runId + "&lastIndex=" + lastIndex,
    function (json) {
      console.log(json);
